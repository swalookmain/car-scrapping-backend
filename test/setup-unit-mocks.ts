/**
 * Mocks Nest services for unit tests to avoid Mongoose schema initialization.
 */
...
const repositoryMocks: Array<[string, string]> = [
  ['src/invoice/invoice.repository', 'InvoiceRepository'],
  ['src/invoice/vehicle-invoice.repository', 'VehicleInvoiceRepository'],
  ['src/invoice/purchase-document.repository', 'PurchaseDocumentRepository'],
];

for (const [path, name] of repositoryMocks) {
  jest.mock(path, () => ({
    [name]: class {},
  }));
}
